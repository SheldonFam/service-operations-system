...
export function DashboardPage() {
  ...
  const { data, isPending, error, refetch } = useDashboard(range)
  const technicians = data?.technicians ?? []
  const summary = data?.summary ?? { total_orders: 0, completed_jobs: 0, pending_review: 0, total_revenue: 0 }
  ...
  return (
    <div className="space-y-6">
      ...
      {error ? (
        <InlineError message="Failed to load dashboard data." onRetry={() => refetch()} />
      ) : isPending ? (
