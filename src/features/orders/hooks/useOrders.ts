...
export function useOrder(id: string) {
  ...
  useEffect(() => {
    ...
    supabase
      .from("orders")
      .select(
        "*, technician:users!assigned_technician(*), service_record:service_records(*, photos:service_photos(*))",
      )
      .eq("id", id)
      .single()
        ...
        if (data) {
          ...
          if (Array.isArray(record.service_record)) {
            record.service_record =
              record.service_record.length > 0
                ? record.service_record[0]
                : null;
          }
        }
...
export function useCreateOrder() {
  ...
  const createOrder = useCallback(
    async (values: OrderFormValues, createdBy: string) => {
      setLoading(true);
      try {
...
export function useUpdateOrder() {
  ...
  const updateOrder = useCallback(
    async (id: string, updates: OrderUpdatable) => {
      setLoading(true);
      try {
...
function loadTechnicians(): Promise<User[]> {
  ...
  techInflight = Promise.resolve(
    supabase.from("users").select("*").eq("role", "technician").order("name"),
  ).then(({ data }) => {
